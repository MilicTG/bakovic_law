...
function BlogCard(props: BlogCardProps) {
    return (
        <Link href={`/blog/${props.blogData.slug}`} key={props.blogData._id}>
            <div
                ...
                <div className="relative h-60 w-full">
                    <Image className="object-cover object-center"
                           src={urlFor(props.blogData.blogImage.asset._ref).width(1000).url()}
                           alt={props.blogData.title}
                           fill={true}/>
                </div>

                <h5 className="text-3xl font-semibold text-primary-color text-center px-4 py-8 font-serif">{props.blogData.title}</h5>
            </div>
        </Link>
    )
}
